...
export default function subTableItem({ data }: any) {
  ...
  return (
    <>
      <div className="container-title bottom-container-title" >Active User Data</div>
      <Card>
        <div className="active-tabs-container">
          {data.tabsList &&
            data.tabsList.map((item: any) => {
              return (
                <div
                  key={Math.floor(Math.random() * 10000 + 1)}
                  className="active-tabs-item"
                >
                  {item}
                </div>
              );
        ...
        {data.processList &&
          data.processList.map((item: any, index: Number) => {
            return (
              <DataItem
                style={
                  index > 0
